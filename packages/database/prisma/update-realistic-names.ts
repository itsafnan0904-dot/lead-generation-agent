...
async function main() {
  console.log('Replacing test fixture company and contact names with realistic B2B enterprise entities in PostgreSQL...');

  const realEnterpriseUpdates = [
    {
      oldDomain: 'stark.com',
  ...
  for (const item of realEnterpriseUpdates) {
    let company = await prisma.company.findFirst({
      where: { OR: [{ domain: item.oldDomain }, { domain: item.domain }] },
    });
    ...
    if (company) {
      ...
      console.log(`Updated company and contact: ${item.name} (${item.contact.email})`);
    }
  }

  console.log('Database updated successfully with realistic B2B enterprise entities.');
